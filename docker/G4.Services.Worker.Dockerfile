...
RUN dotnet restore "G4.Services.Worker/G4.Services.Worker.csproj"

# Copy the remaining source code
COPY . .

# Build the project
RUN dotnet build "G4.Services.Worker/G4.Services.Worker.csproj" -c $BUILD_CONFIGURATION -o /app/build
...
# Expose port 9955 (adjust if needed)
EXPOSE 9955

# Set the entry point for the container
ENTRYPOINT ["dotnet", "G4.Services.Worker.dll"]
